
<img width="1024" height="1024" alt="fc60efe1-ffc0-440a-b710-a9b8a5a7f183-Picsart-BackgroundRemover" src="https://github.com/user-attachments/assets/58989ba6-35da-469c-bd79-caf7c7a6ff2d" />

AMT Menu
<p align="center"> <img src="https://img.shields.io/github/stars/YOUR-USERNAME/REPO-NAME?style=for-the-badge&color=purple" /> <img src="https://img.shields.io/github/forks/YOUR-USERNAME/REPO-NAME?style=for-the-badge&color=blue" /> <img src="https://img.shields.io/github/license/YOUR-USERNAME/REPO-NAME?style=for-the-badge&color=green" /> <img src="https://img.shields.io/github/downloads/YOUR-USERNAME/REPO-NAME/total?style=for-the-badge&color=orange" /> <img src="https://img.shields.io/github/v/release/YOUR-USERNAME/REPO-NAME?style=for-the-badge&color=red" /> </p>

AMT Menu is a feature-packed mod menu for Gorilla Tag, built by me, iiDk.
Whether you just want mods, are a developer, or anything in between, this menu has you covered. Designed to be as useful as possible, it includes a wide variety of features and options that let you customize your modding experience to your heart’s content.

Why settle for boring when you can have stupidly good?
🎬 Preview
<p align="center"> <img src="https://your-gif-link-here/menu-preview1.gif" width="600"/> </p> <p align="center"> <img src="https://your-gif-link-here/menu-preview2.gif" width="600"/> </p>

Replace the GIF links above with:

A menu showcase

Mod toggling preview

UI animation preview

Gameplay feature demo

<details> <summary>💡 <strong>Why open-source?</strong></summary>

AMT Menu is open-source to encourage transparency, learning, and collaboration.
Developers can explore the code, improve it, and contribute new ideas to make the menu even better.

We believe in community-driven development.

</details>
<details> <summary>❓ <strong>Can I use your code?</strong></summary>

Yes — as long as you follow the terms of the GNU General Public License v3.0.

Credit the original author.

Keep derivatives open-source.

Do not claim the work as your own.

</details>
<details> <summary>💾 <strong>Installation</strong></summary>

Download the latest release from the Releases tab.

Extract the files.

Place the .dll file inside your Gorilla Tag Plugins folder.

Launch the game.

⚠️ Make sure you are using the correct mod loader version.

</details>
<details> <summary>🎛️ <strong>System Compatibility</strong></summary>
Requirement	Status
Windows 10/11	✅ Supported
Steam Version	✅ Supported
Mod Loader	✅ Required
Standalone Quest	⚠️ Limited / Experimental
</details>
<details> <summary>🔗 <strong>Headset Compatibility</strong></summary>

Meta Quest (PCVR)

Valve Index

HTC Vive

Oculus Rift

Any SteamVR-compatible headset

</details>
<details> <summary>🗣️ <strong>Contact Information</strong></summary>

GitHub Issues – For bug reports

Discord – (Insert Invite Link)

Email – (Optional)

</details>
💖 Support

If you wish to support me, here are some of the ways you can!

Platform	Link	Address
Bitcoin	BTC	SOON
Ethereum	ETH	SOON
Litecoin	LTC	SOON
XRP	XRP	SOON
Patreon	Patreon	AMTM
⚠️ Disclaimer

This product is not affiliated with Gorilla Tag or Another Axiom LLC and is not endorsed or otherwise sponsored by Another Axiom LLC. Portions of the materials contained herein are property of Another Axiom LLC. © 2026 Another Axiom LLC.

📄 License

Copyright (C) 2026 Ghost Software
https://github.com/somethingtaggers/Amethyst-Menu

This program is free software under the GNU General Public License v3.0.

This program is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY.

See: https://www.gnu.org/licenses/
